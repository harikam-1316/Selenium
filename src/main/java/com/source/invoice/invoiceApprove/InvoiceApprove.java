...
public class InvoiceApprove {
    ...
    public InvoiceApprove(WebDriver driver, WebDriverWait wait, Login login, Logout logout) {
        this.driver = driver;
        this.wait = wait;
        this.login = login;
        this.logout = logout;
    ...
    public void approve(String[] email, String password) throws InterruptedException {
        for (int i = 0; i < email.length; i++) {
            ...
            try {
                ...
                Thread.sleep(2000);
                billlading.clear();
                billlading.sendKeys("9353");
                WebElement update = wait.until(ExpectedConditions.elementToBeClickable(By.id("updateWHTDetails")));
                update.click();
