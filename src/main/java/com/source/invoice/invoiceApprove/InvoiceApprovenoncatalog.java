...
import static com.constants.requisitions.create.PRCreate.BILL_LADING_ID;
import static com.constants.requisitions.create.PRCreate.HEADER_TEXT_ID;

public class InvoiceApprovenoncatalog {
    private WebDriver driver;
    private WebDriverWait wait;
    private Login login;
    private Logout logout;

    public InvoiceApprovenoncatalog(WebDriver driver, WebDriverWait wait, Login login, Logout logout) {
        this.driver = driver;
        this.wait = wait;
        this.login = login;
        this.logout = logout;
    ...
    public void approve(String[] email, String password) throws InterruptedException {
        for (int i = 0; i < email.length; i++) {
            ...
            try {
                ...
                WebElement HeaderText = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id(HEADER_TEXT_ID)));
                HeaderText.clear();
                HeaderText.sendKeys("test");


                WebElement billlading = wait.until(ExpectedConditions.elementToBeClickable(By.id(BILL_LADING_ID)));
                Thread.sleep(2000);
                billlading.clear();
                billlading.sendKeys("8253");

                WebElement updateBtn = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id=\"updateBillHeaderDetails\"]")));
                ...
                try {
                    ...
                    System.out.println("Toast already gone, proceeding with logout...");
                }
