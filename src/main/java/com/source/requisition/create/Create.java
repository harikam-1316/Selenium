...
public class Create {
    ...
    public Create(WebDriver webDriver, Login login, Properties properties, WebDriverWait wait, Logout logout) {
    ...
    public void clickPOCCatalog() {
        try {
            WebElement pocCatalog = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(PRCreate.POC_CATALOG_XPATH)));
            ((JavascriptExecutor) webDriver).executeScript("arguments[0].click();", pocCatalog);
            System.out.println("✅ POC Catalog clicked");
        } catch (Exception e) {
            System.out.println("❌ Error while clicking POC Catalog: " + e.getMessage());
        }
    }
    ...
    public void selectVendor() {
        try {
            ...
            WebElement vendorDropdown = wait.until(ExpectedConditions.elementToBeClickable(By.id(VENDOR_DROPDOWN_XPATH)));
            vendorDropdown.click();

            WebElement vendorSearchBox = wait.until(ExpectedConditions.elementToBeClickable(By.className(VENDOR_SEARCH_BOX_CLASSNAME)));
            vendorSearchBox.sendKeys(vendorName);

            Thread.sleep(3000);
            String vendor = getVendor(vendorName);
            WebElement correctVendorOption = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(vendor)));
    ...
    public void rateContract() {
        try {
            ...
            WebElement rateContractDropdown = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(RATECONTRACT_DROPDOWN_XPATH)));
            rateContractDropdown.click();

            WebElement rateContractSearchBox = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(RATECONTRACT_SEARCH_XPATH)));
            rateContractSearchBox.sendKeys(rateContract);

            Thread.sleep(3000);
            String rateContractXpath = getRatecontract(rateContract);
            WebElement correctrateContractOption = wait.until(ExpectedConditions.elementToBeClickable(By.xpath( rateContractXpath )));
            Thread.sleep(2000);
            correctrateContractOption.click();

            System.out.println("✅rateContract 'NANDI RATE' selected successfully.");

        } catch (StaleElementReferenceException se) {
            System.out.println("⚠️ Stale element, retrying...");
            rateContract();
        } catch (Exception e) {
            System.out.println("❌ Error selecting rateContract: " + e.getMessage());
        }
    }
    ...
    public void SelectShippingMode() {
        try {
            ...
            ((JavascriptExecutor) webDriver).executeScript("window.scrollBy(0, 300);"); // scrolls down 300px
            Thread.sleep(1000);
            ShippingMode.click();
